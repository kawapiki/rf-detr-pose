...
from rfdetr.datasets.synthetic import DatasetSplitRatios, generate_coco_dataset, generate_coco_pose_dataset
...
@pytest.fixture(scope="session")
def synthetic_pose_dataset_dir(tmp_path_factory: pytest.TempPathFactory) -> Generator[Path, Any, None]:
    """Build a synthetic COCO-style pose dataset on disk and clean it up after tests.

    Args:
        tmp_path_factory: Pytest factory for temporary directories.

    Yields:
        Path to the synthetic pose dataset directory.
    """
    seed_all()
    dataset_dir = tmp_path_factory.mktemp("synthetic_pose_dataset")
